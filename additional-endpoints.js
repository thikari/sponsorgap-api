  ...
  try {
    ...
    const [results, totalCount] = await Promise.all([
      sponsors.find(searchQuery)
        .select('name logo link industryfield category audience created')
...
router.get('/sponsors/trending', checkRateLimit, async (req, res) => {
  try {
    ...
    const trending = await sponsors.find({
      $or: [
        { created: { $gte: cutoffDate } },
        { sponsordate: { $gte: cutoffDate } }
      ]
    })
    .select('name logo link industryfield category audience created sponsordate')
